...
test('S22.36 Inventory quantity lock now accepts Edit Inventory permission (not super-admin only)', function() {
  var page = fs.readFileSync(path.join(REPO, 'src/app/page.jsx'), 'utf8');
  // Max: "unless a super admin or permissioned to change it". Previously
  // only super_admin could override. Now anyone with the "Edit Inventory"
  // module permission can too.
  assert(/canOverrideQty = userProfile\?\.role === 'super_admin' \|\| modulePerms\?\.\['Edit Inventory'\] === true/.test(page),
    'form gate widened to accept Edit Inventory permission');
  // Save-side check also widened
  assert(/const canOverrideQty = isSuperAdmin \|\| modulePerms\?\.\['Edit Inventory'\] === true/.test(page),
    'save-flow gate widened');
});
