...
var MANIFEST = [
  ...
  { f: 'test-v55-83-ml-prefill-schema-drift.js', label: 'new', why: 'ML: Step 6 Wave invoice-payment prefill no longer crashes when production is missing optional bank_transactions.wave_transaction_id; deposit read uses select(*) and keeps the safe filter' },
  { f: 'test-v55-83-mm-wave-accounting-export-csv.js', label: 'new', why: 'MM: Wave accounting.csv is double-entry; import uses Cash/Bank rows, signed Amount, Other Accounts as category, and defers A/R payment rows to payment sync' },
  { f: 'test-v55-83-mk-payment-preflight-and-log-scope.js', label: 'new', why: 'MK: payment dry-run uses the real /api/wave/push-payment server preflight, and Sync Log loads active-silo rows first so payment push failures cannot vanish behind unrelated logs' },
  { f: 'test-v55-83-mj-estimate-error-clarity.js', label: 'new', why: 'MJ: optional Wave Estimates -> Proformas importer is collapsed away from the main flow and translates raw Wave/database errors into human guidance' },
  { f: 'test-v55-83-mi-wave-import-reorg.js', label: 'new', why: 'MI: Wave Import is now the single place to pull Wave truth into Hub: chart of accounts, old Wave transaction categories by CSV, invoice payments, customers/invoices; Sync Center no longer has a duplicate Import tab' },
  { f: 'test-v55-83-mn-hubonly-copy-truth.js', label: 'new', why: 'Codex Lane B FAIL: stale Hub-only copy said Wave cannot accept category pushes; now Needs category / categorized DO post / Split not built yet' },
  { f: 'test-v55-83-ms-invoice-product-redesign.js', label: 'new', why: 'Codex Round-3 spec: combined Approve&Push + catalog-first Default Invoice Product (deltas 1-5) + prerequisite ladder + Round-2 category fixes' },
  { f: 'test-v55-83-ni-unmatch-splits.js', label: 'new', why: 'NI (Max Aug 25, deposit 271758): unmatch voided a split deposit\'s payments+matches but left bank_transaction_splits rows and reviewed status - deposit displayed fully allocated while every payment was dead. Unmatch now deletes split lines, voids open unapplied parks, and drops the txn to unreviewed. Wave-synced block + per-invoice recompute unchanged' },
  { f: 'test-v55-83-nf-inventory-reports.js', label: 'new', why: 'NF (Max Aug 12): inventory report suite - Stock & P&L (original qty, rolls rcvd, sold qty/rolls, on hand, avg sale price, revenue, avg cost, COGS, profit, margin, cost status), Customer Copy (pnl columns ABSENT from rows, not masked), Consolidated by family (LUX as one number; averages re-derived from sums). Also removed the pre-MX pending-on-top-of-layers double-count from the Report Center' },
  { f: 'test-v55-83-ne-stock-count.js', label: 'new', why: 'NE (Max Aug 12): physical stock count - the correction for stock overstated by every pre-MX sale (links unrecoverable). NEEDS SQL v55-83-NE-stock-count.sql which ALSO installs the original adjustments migration (audit proved it never ran; the Adjustments screen has been calling missing functions). Shrinkage reduces open layers oldest-first w/ adjustment_out movements; found stock enters via COUNT-ADJ receipt -> provisional layer; rolls stay general counts folded into Overview arithmetic; empty rows never touched' },
  { f: 'test-v55-83-nd-reconciliation-center.js', label: 'new', why: 'ND (Max Aug 12): Wave-Hub reconciliation with per-line Match Hub/Match Wave queue + one SUBMIT. Read-only diff route (invoices + payments, all 4 diff types, worst gaps first); executor reuses guarded routes only (push/replace/delete-invoice/import/push-payment; voids never deletes payments); impossible directions BLOCKED with reasons (Hub never deletes Wave-authored invoices or Wave payments); compare re-runs after submit as proof' },
  { f: 'test-v55-83-nc-delete-invoice-wave.js', label: 'new', why: 'NC (Max Aug 11): delete an invoice from Accounting AND from Wave. Wave-first ordering (refusal = nothing deleted anywhere; worst crash = Hub row without Wave copy, finishable with normal Delete - never an orphaned live Wave invoice). Wave-pushed payments hard-block; Hub-only payments need explicit second ack; wave_import refused; lifecycle blocker on plain delete STAYS' },
  { f: 'test-v55-83-nb-currency-line-edit-delete.js', label: 'new', why: 'NB (Max Aug 11): edit + delete on other-currency Treasury lines. The edit modal covered only EGP cash + bank sides; wrong usd_in/usd_out/foreign_amount was uncorrectable from the UI. New currency section in the SAME modal (all existing guards apply: delete confirm, super-admin/creator-24h, invoice recalc, check revert). Sealed-bucket + trio-consistency guards: amount without currency code refuses to save; clearing amount clears code+direction; bank rows never show or write currency fields' },
  { f: 'test-v55-83-na-amend-update-wave.js', label: 'new', why: 'NA (Max Aug 11): amend a pushed invoice and update Wave. Wave has no invoiceUpdate; new /api/wave/replace-invoice deletes the old Wave copy + unlinks (recreate delegated to push-invoice-v2 - single source of truth). MONEY GUARDS: payments already in Wave block the replace; Wave-imported invoices are never deleted; unlink is immediate after confirmed delete so two copies can never exist' },
  { f: 'test-v55-83-mz-treasury-currency-toggle.js', label: 'new', why: 'MZ (Max Aug 11): Treasury cards/table/balance were EGP-only; USD + foreign ledgers sat in separate sections far down the page. Now a currency toggle drives the three buckets, a backing transaction list, and the running balance. CRITICAL INVARIANT: currencies are never summed and no FX is applied - cash_in/cash_out (EGP), usd_in/usd_out (USD), foreign_amount+foreign_direction (rest) stay sealed' },
  { f: 'test-v55-83-my-product-history-sales.js', label: 'new', why: 'MY (Max Aug 11): Product History > Sales listed only inventory_movements (no order #, no customer) so it could not answer "which order took my stock". Now reads invoice_items directly + joins order_number/customer. Also fixed two silent query failures: movements ordered by nonexistent moved_at, layers by nonexistent received_at - Postgres rejected both ORDER BYs, so the tabs rendered empty even with data' },
  { f: 'test-v55-83-mx-inventory-deduction.js', label: 'new', why: 'MX (Max Aug 10): sales invoices were not deducting stock. Layers only existed after cost-finalize, but Overview showed un-finalized stock as on-hand, so selling it deducted 0 and booked COGS 0. Now layers are created on arrival (provisional, cost 0), finalize updates the SAME layer and restates earlier sales P&L, delete/void reverses for real, sales write a movements row, and Overview no longer adds pending receipts on top of layers. Also the Step-4 variance panel contrast fix' },
  { f: 'test-v55-83-mw-header-net-hidden.js', label: 'new', why: 'MW (Max Aug 10): header NET amount is masked by default with an eye toggle. Registered HERE despite being a header/UI test because this is the only gating runner we have — and the one thing that must never silently regress is useState(false). A flip to true, or a localStorage persist, re-exposes the company cash position on screen-shares' },
  { f: 'test-v55-83-mv-openaccounts-running-currency.js', label: 'new', why: 'MV (Max Jul 21): Open Accounts ledger running column now shows the ROW\'S OWN currency net with currency stamp — per-currency running columns clipped EGP off-screen so EGP rows appeared to carry the USD constant' },
  { f: 'test-v55-83-mu-bankreview-duplicates.js', label: 'new', why: 'MU (Codex build, Claude QA): Accounting Bank Review relink-duplicate guard — high-confidence groups only (same fingerprint across >1 account_id), money-carrying rows protected as keepers, mark_review_duplicates server action recomputes groups server-side and never deletes/voids' },
  { f: 'test-v55-83-mt-synclog-doubled-multipush.js', label: 'new', why: 'Codex Round 4 think-tank: Sync Log 3-state chip + invoice log context + mojibake fix; removed duplicate invneedsapproval row; multi-push lifted (sequential, dependency order, visible inline)' },
  { f: 'test-v55-83-ms-csv-amount-column.js', label: 'new', why: 'Codex Round-2 real fixture: signed Amount (One column) never mis-detected as Debit Amount (One column)' },
  { f: 'test-v55-83-mo-categories-paginated.js', label: 'new', why: 'only 33 categories showed instead of all: /api/wave/categories now paginates the wave_categories read (Supabase 1000-row cap was dropping real categories in a 1877-row silo)' },
  { f: 'test-v55-83-mm-wave-gl-csv-import.js', label: 'new', why: 'import old Wave categories now works with Waves REAL Account-Transactions export: reads the Other Accounts for this Transaction column, processes only bank-side rows (skips the duplicate category-side row), trusts the signed Amount column (GL debit/credit is inverted), skips Uncategorized' },
  { f: 'test-v55-83-mk-prefill-resilient-deposit-read.js', label: 'new', why: 'live: Preview deposit links errored "column bank_transactions.wave_transaction_id does not exist" — prefill deposit read now select(*) so a missing LF-migration column does not break Step 6' },
  { f: 'test-v55-83-mi-feed-owner-grouping.js', label: 'new', why: 'MI: who-feeds-each-account groups duplicate-named Wave Cash/Bank accounts; one click sets the owner on ALL underlying ids (wave_account_ids[]) so the firewall does not block sibling duplicates' },
  ...
  { f: 'test-v55-83-lp-guided-prefill.js', label: 'new', why: 'guided Prefill-from-Wave flow lives in Wave Import: import invoices+customers, categories CSV, prefill invoice links Preview-first; UI orchestration of existing tested routes, nothing writes to Wave' },
  ...
  { f: 'test-v55-83-ld-import-wave-csv.js', label: 'new', why: 'PULL existing Wave categorizations (Max item 2): ingest Waves CSV export in Wave Import (API cannot read old categorized transactions), match to Hub txns by date+amount+description, resolve category name->id, reflect as already-in-Wave; dry-run default' },
