              ...
              return (
                <div className="rounded-lg p-3 mb-4" style={{ background: s.bg }}>
                  <div className="text-sm font-bold" style={{ color: s.color }}>
                    ...
                    {status === 'mismatch' && ' — Sales: ' + fE(selectedInvoice.total_collected) + ' vs Treasury: ' + fE(tTotal)}
                  ...
                  {(status === 'unverified' || status === 'mismatch') && (
                    <div className="text-xs mt-1" style={{ color: s.color }}>
                      {status === 'unverified' ? 'Use "Link Transaction" below to find and link the matching treasury payment' : 'Treasury amount differs from sales record. Link/unlink transactions to correct.'}
                    </div>
                  )}
                </div>
              );
                        ...
                        editingTxn === txn.id ? (
                          <tr key={txn.id} className="bg-blue-50">
                            ...
                            <td className="px-2 py-1.5">
                              ...
                              <select defaultValue={txn.category || 'Operations'}
        ...
        {showAddTreasury && (
          <Modal onClose={() => { setShowAddTreasury(false); setFormData({}); }} title="New Transaction / معاملة جديدة">
            <div className="grid grid-cols-2 gap-3">
              ...
              {(formData.type || 'in') === 'out' && (
                <div className="col-span-2">
                  <label className="text-xs font-semibold text-slate-600">Expense Category / تصنيف</label>
