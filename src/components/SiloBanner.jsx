...
export default function SiloBanner(props) {
  ...
  var notConnected = props.notConnected;    // v55.83-KT: silo is on a placeholder Wave id (not bound yet)
  var label = props.label || 'No business selected';

  // Palettes — dark bg, bright border, white text in every case.
  var palette;
  if (!registered || notConnected) {
    ...
    <div style={{
      ...
      <div style={{ color: '#ffffff' }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 600, marginTop: 2, color: 'rgba(255,255,255,0.92)' }}>
          {notConnected
            ? 'Not connected to a real Wave business yet — bind it in Wave Connection. All Wave actions are blocked until then.'
            : registered
      ...
      {registered && (
        <div style={{
          ...
          {notConnected ? '\u26a0 NOT CONNECTED TO WAVE' : ((isTest ? 'TEST' : 'PRODUCTION') + ' \u00b7 ' + (canWrite ? 'READ-WRITE' : 'READ-ONLY'))}
        </div>
      )}
    </div>
  );
