...
export const BUILD_HISTORY = [
  {
    ...
    items: [
      '**🩹 HOTFIX 11 (May 26 2026):** Open Accounts table rebuilt to standard accounting layout after feedback from Max and a third-party reviewer. The previous "signed Amount" column was confusing — a Payment Received of 32,888 USD was showing as a large red −32,888, even though money was flowing INTO the company. And the Totals row was summing assets (sales invoices owed to us) with liabilities (vendor bills we owe) as positive magnitudes, which is mathematically meaningless. All fixed:',
      '   • **Two-column money display.** The single Amount column is now split into "Amount In" (green) and "Amount Out" (red). Every value is shown as a positive number; the direction is conveyed by which column is filled. Payment Received goes in Amount In. Payment Sent goes in Amount Out. Standard accounting. No more hostile minus signs.',
      '   • **Open AR and Open AP separated.** The single Remaining column is split into "Open AR" (sales invoices still owed to us, emerald) and "Open AP" (vendor bills we still owe, red). Each row fills only the column it belongs in. The Totals row at the bottom adds them separately — never blindly summed together.',
      '   • **Net Position row added.** A dedicated dark row at the very bottom of each account spells out: "Open AR − Open AP = Net Position" with all three values shown explicitly. No more guessing how the Total Remaining number was derived.',
      '   • **Net USD / Net EGP renamed to Running USD / Running EGP.** Those columns were always cumulative running balances after each row, not the net impact of that specific row. The label was misleading. Now correctly labelled.',
      '   • **NEW: Global Net Position card at the top of the tab.** Shows your total liquidity converted to a single base currency (USD). It picks up FX rates from the FX Rates table you maintain in Inventory and converts every other currency to USD-equivalent, then sums them all up. So at a glance you can see whether you are net asset or net liability across the entire business in one number. Shows each currency contribution with the exact rate used, and warns you if any FX rate is missing.',
      '   • Print Statement (Internal and Customer perspective) and Excel export both updated to the new layout. Customer perspective correctly swaps In/Out and AR/AP for the recipient.',
