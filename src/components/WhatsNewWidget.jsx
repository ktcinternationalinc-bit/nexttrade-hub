...
export const BUILD_HISTORY = [
  {
    ...
    items: [
      '**🩹 HOTFIX 11 (May 26 2026):** Open Accounts table fully rebuilt to standard accounting layout after iterating with Max and a third-party review pass:',
      '   • **AR Side / AP Side columns.** The single Amount column is replaced with two columns named "AR Side" (emerald) and "AP Side" (red). Each row goes in exactly one column based on which ledger account it affects. Sales Invoice → AR Side (creates AR). Payment Received → AR Side (reduces AR). Vendor Bill → AP Side (creates AP). Payment Sent → AP Side (reduces AP). All numbers shown as positive; the Type column tells you whether the row is increasing or reducing that side.',
      '   • **Single Remaining column.** Replaces the previous attempt at Open AR / Open AP split columns. The Remaining column fills only on invoice or bill rows and is colored by which side it belongs to (emerald for AR, red for AP). Settled invoices show "✓ paid".',
      '   • **Per-currency Summary block (replaces the old totals row).** At the bottom of each account, instead of an inline totals row that mixed assets with liabilities, you now see a clean Summary block per currency with three rows: "Total AR (They Owe Us)" / "Total AP (We Owe Them)" / "Net CUR Position: AR − AP = result". The Net Position row spells out the arithmetic explicitly so there is no guesswork. A sub-label says "in our favor" if positive or "against us" if negative.',
      ...
      '   • **NEW: Global Net Position card at the top.** Pulls the most recent FX rates from your existing FX Rates table (Inventory → FX Rates) and converts every non-USD balance into USD equivalent, then sums them all into one base-currency number. So at a glance you see your total liquidity across every counterparty and every currency, in USD. Shows each currency contribution with the exact rate used (e.g. "−20,500.00 EGP × 0.020408 = −418.37 USD"). If any FX pair is missing, the card flags it with an amber warning telling you to add it.',
      '   • **Print Statement and Excel export updated.** Both use the same AR Side / AP Side / Remaining / Running Balance layout. The Customer Statement perspective correctly swaps AR/AP for the recipient.',
      '   • Tests: 47 assertions covering all helpers, table headers, per-row routing, Summary block, Net Position arithmetic, "in our favor / against us" sub-labels, FX conversion math (with the spec example: USD −2,876 + EGP −20,500 ÷ 50 = USD −3,286 global net), and print + Excel exports.',
