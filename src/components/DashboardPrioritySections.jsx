...
export default function DashboardPrioritySections({
  ...
  // If all three are empty, show a single quiet success card and bail.
  if (overdue.length === 0 && recentUpdates.length === 0 && newlyAssigned.length === 0) {
    return (
      <div className="mb-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
        <div className="text-2xl mb-1">✅</div>
        <div className="text-sm font-bold text-emerald-900">You're all clear / لا توجد مهام عاجلة</div>
        <div className="text-xs text-emerald-700 mt-0.5">No overdue tickets, no new assignments. Nice work.</div>
      </div>
    );
  }
