...
ok('2: multi-account silo anchor safety — block transaction push when the silo has >1 bank account',
  /from\('plaid_accounts'\)\.select\('mask, plaid_account_id'\)\.eq\('wave_business_id', waveBusinessId\)/.test(route) &&
  /if \(Object\.keys\(masks\)\.length > 1\) \{ return blocked\(/.test(route));
