...
ok(/function isDraftStatus\(st\) \{ return st === 'DRAFT' \|\| st === 'SAVED'/.test(imp),'import knows Wave draft statuses');
...
// dashboard
ok(/i\.wave_status !== 'DRAFT' && i\.wave_status !== 'SAVED'/.test(dash),'dashboard AR excludes drafts');
...
// AR history
ok(/function isDraft\(inv\)/.test(arh)&&/if \(isDraft\(i\)\) return/.test(arh),'AR History excludes drafts');
