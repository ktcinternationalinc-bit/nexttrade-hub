...
var reg=[{wave_business_id:'A'},{wave_business_id:'B'}];
...
ok(ex.scopeIfRegistered(rows,'A',reg,true).map(function(r){return r.id;}).join(',')==='1,3','registered A => A + legacy null only');
ok(ex.scopeIfRegistered(rows,'B',reg,false).map(function(r){return r.id;}).join(',')==='2','registered B (no legacy) => only B');
// usage in all views
