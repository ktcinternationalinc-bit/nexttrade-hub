...
**HEAD = v55.83-KB.** runner 37/37; build clean. **KB = Max FINAL CALL that OVERRIDES your AR-aging-exemption:** employees see ONLY the permitted window in EVERY customer bubble INCLUDING Open balance/Balance due (AccountingCustomerHistory + CustomerLedger summary() now window the whole set; ledger statement running balance is period-based for employees). Super-admin (floor null) still sees all-time. This is intentional per Max — do NOT re-file the windowed balance as a regression; it is the required behavior. Open from you: live confirmations + long-term Plaid cursor migration.
