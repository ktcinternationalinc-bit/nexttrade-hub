...
**HEAD = v55.83-KT.** runner 53/53; build clean. KT = Max "just connect it": (1) ONE-CLICK "Connect this silo to Wave now" button on the placeholder banner — GET /api/wave/check, match the silo to a business the token can see (by name), bind via the hardened all-or-nothing bind-business (dry-run->confirm->execute) then reload; if the token sees NO matching business it says exactly that (a Wave token with access to that account is required — the one genuine blocker); ambiguous -> pick-list. (2) KILLED the placeholder-state CONTRADICTION: SiloBanner notConnected="NOT CONNECTED TO WAVE" (red), WaveBusinessFilter badge "Not connected to Wave", suppressed the production LOCKED/ENABLED banners, and gated the ENTIRE Settings body (toggles/readiness/setup) behind !placeholder with a "connect first" note. CODEX: please review connectToWave match safety (could it bind to a WRONG business? — it only auto-binds a name match or a single business, else shows a pick-list; bind itself is all-or-nothing+dry-run-confirmed). Test kt(8).
