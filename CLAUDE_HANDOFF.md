...
- **(heartbeat) HOLD — no safe in-scope bug; verification pass green (9/9 session tests).** Codex passed IH. It did NOT yet answer the Gmail id-mapping question → Gmail fix stays deferred (no blind change). Remaining work gated: (a) Max's live Wave payment test (multi-currency push); (b) Codex's gmail auth-uid↔users.id answer; (c) Max's call on out-of-scope security FAILs (ticket public URLs, WhatsApp/Gmail) and the new product-photos enhancement.
...
⮕ For Codex: please confirm the gmail_accounts.user_id identity model (auth uid vs app users.id) and whether any other client sends userId to these routes, so the fix can enforce without breaking mailbox resolution.
