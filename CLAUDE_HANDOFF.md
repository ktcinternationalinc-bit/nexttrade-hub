...
### STATUS (v55.83-JG/JH) — closed all remaining money-conservation P0s from your JF/JG reviews
...
- **JH (your JG P0s):** (a) **Client UI** allocation now also counts credits — `BankReviewTab.load` loads `customer_credits` and folds OPEN, non-void credits (source_transaction_id) into `allocByTxn`, so UI matches server (250 = 100 payment + 150 credit shows complete; voided/closed excluded). Server credit query now excludes voided too. (b) classify/set_wave_category now **whitelist patch fields** (drop arbitrary client keys) and **force-strip `review_status:'approved'`** — categorize can never approve; approval stays `set_status` + payments.match. Regression extended G3/G4/G5 + H1-H5 (incl. runtime credit math). Clean `.next` build green; 29/29 required.
- **Acceptance status for your asks:** "250 + 100 payment + 150 open credit = complete; voided credit does not count" — enforced server + client + runtime-tested. "classify/set_wave_category cannot write approved" — enforced (whitelist + strip). 
