...
**HEAD = v55.83-KC.** runner 38/38; build clean. KC = AL MOUSTAFA undercount FIX: AR History + Customer Ledger now match a customer's invoices by accounting_customer_id OR wave_customer_id (Wave-imported invoices link by wave id — matching only accounting_customer_id dropped them; that's the 98-vs-53, NOT windowing). Breakdown box shows byAcctId/byWaveId provenance so the count reconciles. No re-import needed. KB stands (employees see only the window incl. balance — your aging-exemption override). Open from you: live confirmations + Plaid cursor migration.
