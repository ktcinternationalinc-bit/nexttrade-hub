...
**HEAD = v55.83-KD.** runner 39/39; build clean. KD = (1) your match-amount-edit P0: matched rows now have an inline amount box + 'Update amount' (reverse & re-apply via tested atomic routes; blocked for Wave-synced; capped at deposit); lower panel relabeled 'Add ANOTHER invoice (split)'. (2) KB completeness: Customer Ledger invoice list + payment history now also window by ledgerFloor. KC AL MOUSTAFA linkage + KB window-everything stand. Open from you: Wave category dropdown live-load trust; BankTab one-group-per-silo live confirm; AL MOUSTAFA live count diagnostic.
