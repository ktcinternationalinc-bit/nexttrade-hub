...
-- =====================================================================
-- Problem: Nadia/Jenna/Sara conversation messages were stored only in
-- the user's browser localStorage. When a user switched from phone to
-- laptop, history started empty — Nadia "forgot" what they said
-- yesterday. The 80-message rolling trim further compounded amnesia
-- for power users.
--
...
COMMENT ON TABLE conversation_logs IS 'v55.81 QA-16: server-side persistence of per-persona conversation tail. Backs cross-device continuity. Trimmed to 80 messages per persona per user.';

COMMENT ON COLUMN conversation_logs.messages IS 'Array of {role: user|assistant, content: text, ts: ISO datetime}. Capped at 80 entries (matches client localStorage trim).';
